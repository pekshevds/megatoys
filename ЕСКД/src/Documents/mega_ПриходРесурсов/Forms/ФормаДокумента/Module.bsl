...
Процедура СкрытьСтолбцы()
	...
	Для индекс = 1 По 31 Цикл
		...
		Элементы["ПроизводственныеРесурсыВремя" + Формат(индекс, "ЧГ=0; ДФ=d;")].Видимость = Ложь;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОчиститьВремя()
	...
	Для Каждого ТекСтрокаПроизводственныеРесурсы Из Объект.ПроизводственныеРесурсы Цикл
		Для индекс = 1 По 31 Цикл
			...
			ТекСтрокаПроизводственныеРесурсы["Время" + Формат(индекс, "ЧГ=0; ДФ=d;")] = 0;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура УстановитьЗаголовкиСтолбцов()
	...
	ПериодМесяц = ?(Объект.ПериодМесяц = 0, Месяц(ТекущаяДатаСеанса()), Объект.ПериодМесяц);
	
	СкрытьСтолбцы();	
		
	ТекущийДень = Дата(ПериодГод, ПериодМесяц, 1, 0, 0, 0);
	ПоследнийДень = НачалоДня(КонецМесяца(ТекущийДень));
	Пока ТекущийДень <= ПоследнийДень Цикл
		
		Столбец = Элементы["ПроизводственныеРесурсыВремя" + Формат(ТекущийДень, "ЧГ=0; ДФ=d;")];		
...
Процедура ЗаполнитьПоУмолчаниюНаСервере()
	...
	Для Каждого ТекСтрокаПроизводственныеРесурсы Из Объект.ПроизводственныеРесурсы Цикл
		...
		Пока ТекущийДень <= ПоследнийДень Цикл
			...
			ТекСтрокаПроизводственныеРесурсы["Время" + Формат(ТекущийДень, "ЧГ=0; ДФ=d;")] = 0;			
			Если Справочники.ПроизводственныеКалендари.ЭтоРабочийДень(ДанныеПроизводственногоКалендаря, ТекущийДень) Тогда
				
				ТекСтрокаПроизводственныеРесурсы["Время" + Формат(ТекущийДень, "ЧГ=0; ДФ=d;")] = 
